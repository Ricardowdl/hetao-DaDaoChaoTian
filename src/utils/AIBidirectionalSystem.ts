...
class AIBidirectionalSystemClass {
  ...
  public async processPlayerAction(
    ...
    const isRealTavernEnv = isTavernEnv();
...
`.trim();
      ...
      // 🔥 关键修复：getTavernHelper 在非酒馆也会返回包装器，不能用 tavernHelper 是否存在来判断环境。
      // 非酒馆（自定义API）时必须把 onStreamChunk 传下去，才能实时更新 uiStore.streamingContent。
      const response = await tavernHelper.generate({
        user_input: finalUserInput,
        should_stream: useStreaming,
        generation_id: generationId,
        injects: injects as any,
        ...(isRealTavernEnv ? {} : { onStreamChunk: options?.onStreamChunk }),
      } as any);
