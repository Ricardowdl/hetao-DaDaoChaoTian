            ...
            <div class="setting-item">
              ...
              <div class="setting-control">
                ...
                >
                  ...
                  <option value="official">{{ t('官方端点') }}</option>
                  <option value="custom">{{ t('自定义(OpenAI兼容)') }}</option>
                </select>
              </div>
            </div>

            <div v-if="aiConfig.customAPI.provider !== 'official'" class="setting-item setting-item-full">
              <div class="setting-info">
...
const onProviderChange = () => {
  ...
  if (preset && aiConfig.customAPI.provider !== 'custom') {
    // 官方端点：URL 在 UI 中完全隐藏且不允许查看/修改，因此不写入配置
    aiConfig.customAPI.url = aiConfig.customAPI.provider === 'official' ? '' : preset.url;
...
const exportSettings = () => {
  ...
  try {
    ...
    link.download = `纵横诸天-设置备份-${dateStr}.json`;
