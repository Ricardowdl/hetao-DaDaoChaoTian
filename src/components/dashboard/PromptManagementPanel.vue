<template>@@
  <div class="prompt-panel">
    <div class="panel-header compact">
      ...
      <div v-if="!isInGamePrompts" class="panel-actions">
      ...
      <div v-for="(categoryData, categoryKey) in promptsByCategory" :key="categoryKey" class="category-section">
        ...
        <div class="category-header" @click="toggleCategory(categoryKey)">
          <div class="category-title">
            <span v-if="!isInGamePrompts" class="category-icon">{{ categoryData.info.icon }}</span>
            <span class="category-name">{{ categoryData.info.name }}</span>
            <span v-if="!isInGamePrompts" class="category-count">{{ categoryData.prompts.length }} 个提示词</span>
          </div>
          <div class="category-actions">
            <span v-if="!isInGamePrompts" class="category-desc">{{ categoryData.info.description }}</span>
            <svg
              class="expand-icon"
          ...
          <div v-for="prompt in categoryData.prompts" :key="prompt.key" class="prompt-item">
            <div class="prompt-header" @click="togglePrompt(prompt.key)">
              <div class="prompt-title-area">
                <!-- 显示顺序号（仅核心请求提示词） -->
                <span v-if="!isInGamePrompts && categoryKey === 'coreRequest' && prompt.order" class="prompt-order">
                  {{ prompt.order }}
                </span>
                <span class="prompt-title">{{ prompt.name }}</span>
              </div>
              <div v-if="!isInGamePrompts" class="prompt-meta">
            ...
            <div v-if="!isInGamePrompts && expandedPrompts[prompt.key]" class="prompt-content">
...
const isInGamePrompts = computed(() => router.currentRoute.value.path.startsWith('/game'));
...
function togglePrompt(key: string) {
  if (isInGamePrompts.value) return;
