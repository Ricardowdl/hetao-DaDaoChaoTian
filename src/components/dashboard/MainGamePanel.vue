...
const autoScrollThresholdPx = 40;
...
const handleContentAreaScroll = () => {
  const el = contentAreaRef.value;
  if (!el) return;
  const distanceToBottom = el.scrollHeight - el.scrollTop - el.clientHeight;
  shouldAutoScroll.value = distanceToBottom <= autoScrollThresholdPx;
};

watch(contentAreaRef, (newEl, oldEl) => {
  if (oldEl) oldEl.removeEventListener('scroll', handleContentAreaScroll);
  if (newEl) newEl.addEventListener('scroll', handleContentAreaScroll, { passive: true } as AddEventListenerOptions);
  nextTick(() => handleContentAreaScroll());
});

onUnmounted(() => {
  if (contentAreaRef.value) {
    contentAreaRef.value.removeEventListener('scroll', handleContentAreaScroll);
  }
});
...
watch(isAIProcessing, async (processing) => {
...
onMounted(async () => {
  try {
    ...
    if (isTavernEnvFlag) {
      ...
      const getEventBridge = () => {
        const w = window as any;
        try {
          return {
            eventOn: w.eventOn || w.parent?.eventOn || w.top?.eventOn,
            eventOff: w.eventOff || w.parent?.eventOff || w.top?.eventOff,
            tavern: w.TavernHelper || w.parent?.TavernHelper || w.top?.TavernHelper
          };
        } catch {
          return {
            eventOn: w.eventOn,
            eventOff: w.eventOff,
            tavern: w.TavernHelper
          };
        }
      };

      const { eventOn, tavern } = getEventBridge();
      const iframe_events = tavern as Record<string, unknown> | undefined;
      ...
      if (eventOn && iframe_events && typeof eventOn === 'function' && !listenersRegistered) {
        ...
        globalHandlers.onStreamToken = (...args: unknown[]) => {
          let chunk = '';
          let generationId: string | null = null;

          if (args.length === 1 && args[0] && typeof args[0] === 'object') {
            const payload = args[0] as Record<string, unknown>;
            const token = payload.chunk ?? payload.token ?? payload.text ?? payload.content;
            chunk = typeof token === 'string' ? token : '';
            const gid = payload.generation_id ?? payload.generationId ?? payload.id;
            generationId = typeof gid === 'string' ? gid : null;
          } else {
            chunk = typeof args[0] === 'string' ? args[0] : '';
            generationId = typeof args[1] === 'string' ? args[1] : null;
          }

          if (!useStreaming.value || !chunk) return;
          if (generationId && generationId !== currentGenerationId.value) return;
...
onUnmounted(() => {
  ...
  try {
    const w = window as any;
    let eventOff = w.eventOff;
    let iframe_events = w.TavernHelper as Record<string, unknown> | undefined;
    try {
      eventOff = w.eventOff || w.parent?.eventOff || w.top?.eventOff;
      iframe_events = (w.TavernHelper || w.parent?.TavernHelper || w.top?.TavernHelper) as Record<string, unknown> | undefined;
    } catch {
      // ignore
    }
