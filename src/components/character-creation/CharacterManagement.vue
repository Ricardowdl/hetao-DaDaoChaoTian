<template>
  <div class="character-management-panel" :class="{ fullscreen: isFullscreen }">
    ...
    <main class="main-content" :class="{ 'fullscreen-content': isFullscreen }">
      ...
      />
      <!-- 返回按钮 - 仅在全屏模式显示 -->
      <div v-if="isFullscreen && !isMobile" class="fullscreen-header">
      ...
      <div class="mobile-header">
        <button v-if="isFullscreen" class="mobile-back-btn" @click="handleClose">
          <ArrowLeft :size="18" />
        </button>
        <button
        ...
        >
          ...
          <span class="menu-text">{{ isCharacterPanelOpen ? $t('返回') : $t('角色列表') }}</span>
        </button>
      ...
      <div v-if="Object.keys(characterStore.rootState.角色列表).length === 0" class="empty-state">
        ...
        <div class="empty-actions">
          <button @click="goBack" class="btn-create">{{ $t('踏入纵横诸天') }}</button>
        ...
        <div class="grid-container">
          ...
          >
            <div v-if="isMobile" class="mobile-panel-header">
              <button class="mobile-panel-back" @click="isCharacterPanelOpen = false">
                <ArrowLeft :size="18" />
                <span>{{ $t('返回') }}</span>
              </button>
            </div>
...
const exportCharacter = async (charId: string) => {
  ...
  try {
    ...
    link.download = `纵横诸天-角色-${characterName}-${new Date().toISOString().split('T')[0]}.json`;
...
const exportSingleSave = async (charId: string, slotKey: string, slot: SaveSlot) => {
  ...
  try {
    ...
    link.download = `纵横诸天-${saveName}-${new Date().toISOString().split('T')[0]}.json`;
...
const exportSaves = async () => {
  ...
  try {
    ...
    const link = document.createElement('a');
    link.href = URL.createObjectURL(dataBlob);
    link.download = `纵横诸天-${character.角色基础信息.名字}-存档备份-${new Date().toISOString().split('T')[0]}.json`;
