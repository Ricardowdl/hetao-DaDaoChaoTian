  ...
  <div class="creation-container">
    ...
    <div class="creation-scroll">
      ...
      <div class="navigation-buttons">
        ...
        <button
          type="button"
          @click.prevent="(event: Event) => { console.log('[DEBUG] 开启纵横诸天按钮被点击!'); handleNext(event); }"
          :disabled="
            isGenerating ||
            isNextDisabled ||
        ...
        >
          {{ store.currentStep === store.totalSteps ? $t('开启纵横诸天') : $t('下一步') }}
        </button>
      </div>
    </div>
...
@media (max-width: 480px) {
  .creation-scroll {
    width: 100%;
    height: var(--app-height, calc(var(--vh, 1vh) * 100));
    max-height: var(--app-height, calc(var(--vh, 1vh) * 100));
    padding: 1rem;
  ...
  .cloud-sync-container :deep(.cloud-sync-button),
  .cloud-sync-container :deep(.clear-button) {
    width: 100%;
    min-width: 0;
    justify-content: center;
